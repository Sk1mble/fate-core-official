...
Hooks.on ('dropActorSheetData', async (actor, sheet, data) => {
    if (game.user == game.users.find(e => e.isGM && e.active) || game.user.id === data.userid){
        ...
        if (data.type == "track"){
            ...
            if (!data.shift_down){
                if (track?.aspect && track?.aspect !== "No" && track?.aspect !== "no" && track?.aspect != "Name As Aspect" && track?.aspect != "Aspect as Name" && track?.aspect !== game.i18n.localize("fate-core-official.No") && track?.aspect != game.i18n.localize("fate-core-official.AspectAsName") && track?.aspect != game.i18n.localize("fate-core-official.NameAsAspect")){
                    track.aspect.name = "";
                }
